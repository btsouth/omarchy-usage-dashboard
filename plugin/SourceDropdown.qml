...
// emit). Mixing is fine because each row is interpreted independently.
//
...
Item {
  ...
  Column {
    ...
    BorderSurface {
      ...
      // The theme's hover/focus border may only paint the bottom edge. This
      // selector always needs the same complete outline on all four sides.
      readonly property var _borderSpec: Border.flat(
        trigger._focused ? root.accent : Qt.alpha(root.foreground, trigger._hot ? 0.65 : 0.35), 1)
