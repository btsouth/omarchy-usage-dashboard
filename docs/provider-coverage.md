...
The dashboard offers that selection under Filters: a model narrows the whole page without moving off it, and a Models-table row filters and then opens that model's sessions. Everything filtered is genuinely filtered: the summary, the previous-period comparison, the chart, the provider cards, the Go allowance card's model list, and every breakdown, because one selection is applied before any of them is accumulated. The list of models offered is deliberately not narrowed by the filter itself, or it would collapse to the entry already chosen and a model could not be swapped without clearing first; it does follow the period, a chosen day, the route tab, and the account filter.

A source can be left out under Filters. `report --excludeSource <id>` does it for a scripted report; the source is dropped before anything is accumulated, so the summary, the cards, the model filter's own list, and every breakdown agree. This is a view filter and nothing more: settings control visible providers and optional quota requests, while local history remains indexed for later periods. Excluding a source also drops it from the Go allowance card and, when a row had drilled into that source, clears that scope rather than showing an empty page.

The Today chart groups event-timed records into local clock hours. Clicking an hour scopes every breakdown to those records. Hermes `session_model_usage` rows accumulate across a session and are anchored to its first sighting for the daily ledger; they do not identify when individual requests happened. Their tokens count in the day summary and appear as an explicit unplaced total beside the hourly chart. Repeated clock hours at daylight saving transitions carry a time-zone label. The bar panel reads a small snapshot written by each scan and shows its update age; providers with no indexed history are identified rather than displayed as zero usage.
...
CommandCode usage is read from `GET https://api.commandcode.ai/alpha/billing/credits` and `/alpha/billing/subscriptions` with an API key, the same endpoints its own CLI uses. These are not documented for third parties and can change; when a call fails, the previous snapshot stays with its timestamp and an error, and nothing else in the dashboard is affected. The plan's allowance is measured in credit value, not tokens: GOAT allows $14 in any 5 hours, $35 in any 7 days, and $70 a month. Each window reports its own spend, cap, and reset time, so unlike the other providers these meters carry a countdown. The monthly cap is read as remaining credits plus what the period has spent, which the two endpoints agree on exactly, so a plan change needs no code change. Extra credits, the top-up wallet a user buys to keep working once the plan's credits run out, are reported by the same credits response as `purchasedCredits` and shown as a money balance rather than a window: what is left is the endpoint's own figure, and the funded amount it is measured against is that figure plus what `/alpha/usage/summary` reports as drawn from the wallet this period, which is derived and labelled as an estimate. A wallet with nothing left in it reports zero and stays off the card, and one that outlives the plan's windows still shows on its own. Token history comes from T3 Code and Hermes sessions and from Command Code's own session transcripts, and is priced with CommandCode's resale rates, not the labs' list prices. Its key is read from Settings, `COMMANDCODE_API_KEY`, or `~/.config/omarchy/ai-usage/commandcode.key`.
